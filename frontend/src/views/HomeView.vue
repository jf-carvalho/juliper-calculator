    ...
    <div class="grid grid-cols-2 gap-10 h-full">
      <div class="w-full">
        <div class="grid-rows-1 w-full my-3">
          <label for="biggest_salary_value">Month</label>
          <div><input tabindex="1" v-model="month" class="py-2 my-3 px-3 rounded w-full text-gray-900"
              placeholder="Example: january" type="text" /></div>
        </div>

        <div class="grid-rows-1 w-full my-3">
          <label for="biggest_salary_value">First salary</label>
          <div class="grid grid-cols-3 gap-4 my-3">
            <div><input tabindex="2" v-model="first_salary.name" class="py-2 px-3 rounded w-full text-gray-900"
        ...
        <div class="grid-rows-1 w-full my-3">
          ...
          <div class="grid grid-cols-3 gap-4 my-3">
            <div><input tabindex="4" v-model="second_salary.name" class="py-2 px-3 rounded w-full text-gray-900"
                placeholder="Person's name" type="text" /></div>
            <div class="col-span-2"><input @keypress="isNumber($event)" tabindex="5" v-model="second_salary.value"
                class="py-2 px-3 rounded w-full text-gray-900" placeholder="Value" /></div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      if (!this.month) {
        errs.push('Month required')
      }

      if (!this.first_salary.name) {
        errs.push('First salary\'s owner name required')
      }

      if (!this.second_salary.name) {
        errs.push('Second salary\'s owner name required')
      }

      if (!this.first_salary.value) {
        errs.push('First salary\'s value required')
      }

      if (!this.second_salary.value) {
        errs.push('Second salary\'s value required')
      }

      if (this.outflows.length == 1 && (!this.outflows[0].name || !this.outflows[0].value)) {
        errs.push('At least one one outflow required');
      }
      ...
      if (errs.length > 0) {
        ...
        axios.post('http://localhost/save', {
          month: this.month,
          first_name: this.first_salary.name,
          second_name: this.second_salary.name,
          first_salary: this.first_salary.value,
          second_salary: this.second_salary.value,
          outflows: JSON.stringify(this.outflows)
        })
